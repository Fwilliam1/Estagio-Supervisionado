  ...
  const filteredHistory = useMemo(() => {
    return historyList.filter((item) => {
      ...
      const matchSearch =
        ...
        (item.hdrModel &&
          item.hdrModel.toLowerCase().includes(searchTerm.toLowerCase())) ||
        (item.toneMapping &&
          item.toneMapping.toLowerCase().includes(searchTerm.toLowerCase()))
          ...
          <div className="history-filter-tabs">
            ...
            >
              ...
              {historyList.filter((i) => {
                ...
                return !p.includes('super-resolution') && !p.includes('esc') && !p.includes('dmnet') && !p.includes('hdr')
              }).length})
            </button>
            <button
              type="button"
            ...
            >
              ...
              {historyList.filter((i) => {
                const p = String(i.process || '').toLowerCase()
                return p.includes('super-resolution') || p.includes('esc') || p.includes('dmnet')
              }).length})
            </button>
            <button
              type="button"
              ...
              >
                ...
                <div className="history-card-details">
                  ...
                  <div className="history-meta-grid">
                    ...
                    {/* Modelo / Arquitetura se HDR */}
                    {(item.process === 'hdr' || String(item.process || '').toLowerCase().includes('hdr')) && (item.hdrModel || item.model) && (
                      <div className="history-meta-item">
                        <svg
                          width="14"
                          height="14"
                        ...
                        <span>
                          <strong>Modelo HDR:</strong> {item.hdrModel || item.model}
                        </span>
                      </div>
                    )}

                    {/* Tone Mapping se HDR */}
                    {(item.process === 'hdr' || String(item.process || '').toLowerCase().includes('hdr')) && item.toneMapping && (
                      <div className="history-meta-item">
                        <svg
                          width="14"
                          height="14"
                        ...
                        <span>
                          <strong>Tone Mapping:</strong> {item.toneMapping}
                        </span>
                      </div>
                    )}
                  </div>
